...
##
function compile() {
    native_sources=`find $nativessrc_directory'/main' -name '*.c' -o -name '*.cxx' -o -name '*.cpp' -o -name '*.c++'-o -name '*.ino'`
...
function linux_x86_x64() {
    ...
    $gcc -fpie $native_sources -v -o $output_dir'/linux-x86-x64/'${clibName} \
